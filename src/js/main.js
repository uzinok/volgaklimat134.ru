window.onload = function () {
    ...
    function my_slider(section_sl) {
        ...
        function slider() {
            // удаляем сласс 
            if (section_sl.classList.contains("nojs")) section_sl.classList.remove("nojs");

            // все элементы списка
            var items = section_sl.querySelectorAll(".clider-list li");
            if (!section_sl.querySelector(".clider-list .active")) items[0].classList.add("active");

            if (!section_sl.querySelector(".action__navigation button")) {
                ...
                items.forEach(function (item) {
                    ...
                    // смена слайдера по клику
                    button.addEventListener("click", function () {
                        // меняем классы кнопки
                        section_sl.querySelector("button.active").classList.remove("active");
                        this.classList.add("active");
                        // меняем классы Item
                        section_sl.querySelector("li.active").classList.remove("active");
                        console.log(section_sl.querySelector("[data-button='" + this.getAttribute("data-count") + "']"))
                        section_sl.querySelector("[data-button='"+ this.getAttribute("data-count") +"']").classList.add("active");
                    })
        ...
        window.onresize = function () {
            if (document.querySelector("body").clientWidth <= 970) {
                ...
                if (!section_sl.classList.contains("nojs")) section_sl.classList.add("nojs");
                
                if (section_sl.querySelector(".clider-list .active")) section_sl.querySelector(".clider-list .active").classList.remove("active");

                if (section_sl.querySelector(".action__navigation button")) {
                    section_sl.querySelectorAll(".action__navigation button").forEach(function (button) {
                        button.remove();
                    })
                }
            }
    ...
    my_slider(document.querySelector(".nojs.action__js--slider"));
